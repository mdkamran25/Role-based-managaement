    ...
    return (
        <>
            <div className="small-box position-absolute rounded-2">
                {props.task && props.task.map((item, index) => {
                    return (
                        <React.Fragment key={index}>
                            <div className='taskNotification d-flex position-relative justify-content-between pt-2 pb-4 px-3' >
                                ...
                                <p className='text-dark fw-normal mb-0 position-absolute taskName'>{item.taskName}</p>
                                <p className='text-dark fw-normal'>{item.time}</p>
                                <p className='text-dark mb-0 fw-normal position-absolute taskDate'>{item.date}</p>
                            </div>
                            <hr />
                        </React.Fragment>
                    )
